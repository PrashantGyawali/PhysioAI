...
// Original coordinates for the exact image dimensions
...
const bodyParts = body_parts.map((e) => {
    return {
        ...e, // keep the rest of the object properties
        coords: e.coords.split(",") // turn the string into an array
    };
});
...
export function BodyMap() {
    ...
    const [selectedParts, setSelectedParts] = useState([]); // MULTI-SELECT STATE
    ...
    // redraw highlights when selection or image size changes
    useEffect(() => {
        drawHighlights();
    }, [selectedParts]);
    ...
    const toggleSelection = (part) => {
        setSelectedParts(prev =>
            prev.some(p => p.id === part.id)
                ? prev.filter(p => p.id !== part.id) // deselect
                : [...prev, part] // select
        );
    };

    const handleAreaClick = (e, part) => {
        e.preventDefault();
    ...
    const drawHighlights = () => {
        ...
        const scaleX = img.clientWidth / 720; // original width
        const scaleY = img.clientHeight / 789; // original height

        selectedParts.forEach(part => {
            ctx.beginPath();

            for (let i = 0; i < part.coords.length; i += 2) {
                const x = Number(part.coords[i]) * scaleX;
                const y = Number(part.coords[i + 1]) * scaleY;
    ...
    return (
        <div style={{ position: "relative", width: "100%" }}>
            ...
            {/* Debug / Output */}
            <pre>
                {JSON.stringify(selectedParts.map(p => p.title), null, 2)}
            </pre>
